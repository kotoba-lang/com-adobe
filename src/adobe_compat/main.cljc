(ns adobe-compat.main
  "Kotodama WASM entrypoint for the Adobe clean-room actor (L4) — Clojure port.
  ...
  facts (`adobe.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(defn as-int-kernel
  ...
  of adobe-compat.main/as-int stays here as the retained oracle."
  [v]
  (if (pos? v) v 0))
...
(defn emit-facts
  "EAVT facts for one record: {\"adobe.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "adobe-compat" :tier tier :entities entities} 200])
